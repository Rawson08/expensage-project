...
const AddMemberForm: React.FC<AddMemberFormProps> = ({ groupId, onMemberAdded, onCancel, currentMembers }) => {
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800 rounded shadow max-h-[70vh] overflow-y-auto"> {/* Dark mode bg */}
       <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Add Member to Group</h3> {/* Dark mode text */}

       {/* Option 1: Select from Friends */}
       <form onSubmit={handleAddSelectedFriends} className="mb-4 pb-4 border-b dark:border-gray-700"> {/* Dark mode border */}
         <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2"> {/* Dark mode text */}
           Add Existing Friends:
         </label>
         <div className="max-h-40 overflow-y-auto border dark:border-gray-600 rounded p-2 mb-3 space-y-1 bg-gray-50 dark:bg-gray-700/50"> {/* Dark mode list bg/border */}
            {friends.length === 0 ? (
                 <p className="text-xs text-gray-500 dark:text-gray-400 p-2">No friends available to add (or they are already in the group). Add users on the Friends page first.</p>
            ) : (
                friends.map(friend => (
                  ...
                  <div key={friend.id} className="flex items-center">
                    <input
                      ...
                      className="h-4 w-4 text-blue-600 border-gray-300 dark:border-gray-500 rounded focus:ring-blue-500 dark:bg-gray-600 dark:checked:bg-blue-600 dark:focus:ring-blue-600 dark:focus:ring-offset-gray-800" // Dark mode checkbox
                    />
                    <label htmlFor={`friend-${friend.otherUser.id}`} className="ml-2 text-sm text-gray-700 dark:text-gray-200"> {/* Dark mode text */}
         ...
         <button
           ...
           className="w-full inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 dark:focus:ring-offset-gray-800" // Dark mode button
         >
       ...
       <div className="text-center my-2 text-gray-500 dark:text-gray-400 text-sm font-medium">OR</div> {/* Dark mode text */}

       {/* Option 2: Invite New Friend */}
       <div className="space-y-3">
         <label htmlFor="memberEmail" className="block text-sm font-medium text-gray-700 dark:text-gray-300"> {/* Dark mode text */}
         ...
         <div className="flex items-center space-x-2">
             <input
               ...
               className="flex-grow mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm disabled:bg-gray-100 dark:disabled:bg-gray-700 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500" // Dark mode input
             ...
             <button
                ...
                className="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50 dark:focus:ring-offset-gray-800" // Dark mode button
              >
                {friendRequestLoading ? 'Sending...' : 'Send Request'}
              </button>
         </div>
          {addFriendError && <p className="text-sm text-red-600 dark:text-red-400 mt-2">{addFriendError}</p>} {/* Dark mode error text */}
          {addFriendSuccess && <p className="text-sm text-green-600 dark:text-green-400 mt-2">{addFriendSuccess}</p>} {/* Dark mode success text */}
       </div>


       <div className="flex justify-end pt-4 mt-4 border-t dark:border-gray-700"> {/* Dark mode border */}
          <button
             type="button"
             onClick={onCancel}
