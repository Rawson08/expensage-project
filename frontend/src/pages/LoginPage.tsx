...
import { loginUser as serviceLogin } from '../services/authService';
import { AuthUser } from '../context/AuthContext'; // Import AuthUser type if needed
...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const jwtResponse = await serviceLogin({ email, password });

      // Extract user data and token from the response
      const userData: AuthUser = { // Use AuthUser type
          id: jwtResponse.id,
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-end">
            <div className="text-sm">
              <Link to="/forgot-password" className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300"> {/* Dark mode link */}
         ...
         <div className="text-sm text-center">
            <span className="text-gray-600 dark:text-gray-400">Don't have an account? </span> {/* Dark mode text */}
            <Link to="/register" className="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400 dark:hover:text-indigo-300"> {/* Dark mode link */}
              Sign up
            </Link>
          </div>
      </div>
